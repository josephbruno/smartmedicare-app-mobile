...
/// Super admin: manage Prescriptions Under tabs and map medicines into them.
class PrescriptionUnderMasterTab extends StatefulWidget {
  const PrescriptionUnderMasterTab({super.key, this.searchQuery = ''});

  final String searchQuery;

  @override
  State<PrescriptionUnderMasterTab> createState() => PrescriptionUnderMasterTabState();
}

class PrescriptionUnderMasterTabState extends State<PrescriptionUnderMasterTab> {
  static const _unmapped = 'unmapped';

  bool _loadingCats = true;
  bool _loadingProducts = false;
  List<PrescriptionUnderCategoryItem> _categories = [];
  List<Product> _items = [];
  ...
  @override
  void didUpdateWidget(covariant PrescriptionUnderMasterTab oldWidget) {
  ...
  PrescriptionUnderCategoryItem? get _currentCategory {
  ...
  Future<void> _loadCategories({bool keepSelection = false}) async {
    ...
    try {
      final list = await context
          .read<AppServices>()
          .emrMasterData
          .listPrescriptionUnderCategories(includeInactive: true);
      if (!mounted) return;
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      final list = await context.read<AppServices>().emrMasterData.listPrescriptionUnderProducts(
            ...
            ListTile(
              ...
              subtitle: Text(
                _isUnmapped
                    ...
                    : 'Map to ${PrescriptionUnderCategory.labelOf(_currentKey, _categories)}',
              ),
              enabled: !_isUnmapped,
  ...
  Future<void> openAddCategory() async {
    ...
    final ok = await showAppDialog<bool>(
      ...
      builder: (ctx) => AppFormDialogShell(
        title: 'Add Prescriptions Under tab',
    ...
    try {
      final created = await context
          .read<AppServices>()
          .emrMasterData
          .createPrescriptionUnderCategory(label: label);
      if (!mounted) return;
      AppMessenger.show(
        context,
  ...
  Future<void> _editCategory(PrescriptionUnderCategoryItem cat) async {
        ...
        builder: (ctx, setLocal) => AppFormDialogShell(
          ...
          body: Column(
            ...
            children: [
              ...
              if (cat.slug != PrescriptionUnderCategory.unique)
                SwitchListTile(
    ...
    try {
      await context.read<AppServices>().emrMasterData.updatePrescriptionUnderCategory(
            cat.id,
            label: label,
            isActive: isActive,
          );
  ...
  Future<void> _deactivateCategory(PrescriptionUnderCategoryItem cat) async {
    if (cat.slug == PrescriptionUnderCategory.unique) {
      AppMessenger.show(
        context,
    ...
    try {
      await context
          .read<AppServices>()
          .emrMasterData
          .deletePrescriptionUnderCategory(cat.id);
      if (!mounted) return;
      AppMessenger.show(
        context,
  ...
  Future<void> _pickAndAssign(String category) async {
    final selected = await showAppDialog<Product>(
      ...
      builder: (ctx) => _UnmappedMedicinePicker(
        title: 'Map to ${PrescriptionUnderCategory.labelOf(category, _categories)}',
      ),
    );
  ...
  Future<void> _assign(int productId, String? category) async {
    try {
      await context.read<AppServices>().emrMasterData.assignPrescriptionUnderProduct(
            productId,
            category,
          );
      if (!mounted) return;
      AppMessenger.show(
        ...
        SnackBar(
          content: Text(
            category == null
                ? 'Medicine unmapped'
                : 'Mapped to ${PrescriptionUnderCategory.labelOf(category, _categories)}',
          ),
        ),
      );
      await _loadProducts();
  ...
  List<PrescriptionUnderCategoryItem> get _activeCategories =>
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (current.slug != PrescriptionUnderCategory.unique)
                  TextButton(
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(
                            [
                              ...
                              if (p.prescriptionUnderCategory != null)
                                PrescriptionUnderCategory.labelOf(
